...
class ViewModel: ObservableObject {
    ...
    @Published var isShowingResultView: Bool = false
    ...
    init(api: LLMClient, enableSpeech: Bool = false, initialPrompt: String) {
        self.api = api
        ...
        //Task.init {
          //  await self.fetchInitialResponse()
        //}
    }
    
    func updateTranscript(newTranscript: String) {
    ...
    @MainActor
    func fetchInitialResponse() async{
            do {
                
                let combinedInitialPrompt = "\(initialPrompt) \(transcript)"
                let initialResponse = try await api.sendMessage(combinedInitialPrompt)
                //await send(text: initialResponse)
                // Directly append the message to your array
                self.messages.append(MessageRow(
                    isInteracting: false,
                    sendImage: "profile",
                    send: .rawText(transcript),
                    responseImage: api.provider.imageName,
                    response: .rawText(initialResponse),
                    responseError: nil
                ))
